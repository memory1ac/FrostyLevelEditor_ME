...
internal class MeshContainer
{
    ...
    private List<RelocPtr> relocPtrs = [];
    private Dictionary<RelocPtr, string> strings = [];
...
public class MeshSetSection
{
    ...
    private List<ushort> boneList = [];
    private List<float> texCoordRatios = [];
...
public class MeshSetLod
{
    ...
    private List<MeshSetSection> sections = [];
    private List<List<byte>> subsetCategories = [];

    //private uint boneCount;
    private List<uint> boneIndexArray = [];
    private List<uint> boneShortNameArray = [];

    private List<AxisAlignedBox> partBoundingBoxes = [];
    private List<LinearTransform> partTransforms = [];
    ...
    public MeshSetLod(NativeReader reader, AssetManager am)
    {
        ...
        // section categories
        List<long> subsetCategoryOffsets = [];
        for (int i = 0; i < MaxCategories; i++)
        {
            int count = reader.ReadInt();
            subsetCategoryOffsets.Add(reader.ReadLong());
            subsetCategories.Add([]);
...
public class MeshSet : Resource
{
    ...
    private MeshType meshType;
    private MeshLayoutFlags flags;
    private List<MeshSetLod> lods = [];
    private List<uint> unknownUInts = [];

    private ushort boneCount;
    private List<ushort> boneIndices = [];
    private List<AxisAlignedBox> boneBoundingBoxes = [];
