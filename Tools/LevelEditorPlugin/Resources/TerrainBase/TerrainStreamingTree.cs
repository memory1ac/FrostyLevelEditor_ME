...
public class RasterTreeNode
{
    ...
    public List<RasterTreeNode> children = [];
...
public class HeightfieldTree : RasterTree
{
    ...
    public List<RasterTreeNode> nodes = [];
...
#endif
        App.Logger.Log("Node count: {0}, Persistent node count: {1}", nodeCount, persistentNodeCount);
...
public class TerrainMaskTree : RasterTree
{
    ...
    public List<RasterTreeNode> nodes = [];
    public List<TerrainMaskTreeNode.Subtile> subtiles = [];
    ...
    private void loadNodes(NativeReader reader, int id, ref int index, QuadTreeNodeId nodeId, AxisAlignedBox2 nodeCoverage)
    {
        ...
        if (!node.nodeHasBeenPruned && subtileCount > 0)
        {
            ...
            for (int i = 0; i < subtileCount; i++)
            {
                ...
                {
                    terrainLayerIndex = reader.ReadByte(),
                    worldCoverage = reader.ReadAxisAlignedBox2()
                };
    ...
    public override void loadData(NativeReader reader, RasterTreeNode node)
    {
        ...
        // 计算实际需要的元素数量（依据 flags）
        int expectedCount = mnode.subtiles.Count(s => (s.flags & 2) == 0);

        // 如果元数据不一致，记录并使用实际需要的数量
        if (expectedCount != mnode.nonTrivialSubtileCount)
        {
            // 记录警告（替换为你的日志机制）
            App.Logger.LogWarning($"nonTrivialSubtileCount mismatch: meta={mnode.nonTrivialSubtileCount}, actual={expectedCount}");
        }
        mnode.data = new byte[expectedCount][];
...
public class TerrainMaterialTree : RasterTree
{
    ...
    public List<RasterTreeNode> nodes = [];
    public List<uint> materialPairIndices = [];
...
public class TerrainStreamingTreeNode
{
    ...
    public List<TerrainStreamingTreeNode> children = [];
    public QuadTreeNodeId id;
}

public class TerrainStreamingTree : Resource
{
    List<Type> rasterTreeTypeList =
    [
    ...
    List<RasterTree> rasterTrees = [];
    List<TerrainStreamingTreeNode> streamingNodes = [];
