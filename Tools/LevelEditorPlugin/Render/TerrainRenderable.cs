...
public class TerrainRenderable
{
    ...
    protected List<TerrainChunkRenderable> terrainChunks = [];
    ...
    public TerrainRenderable(RenderCreateState state, TerrainStreamingTree streamingData)
    {
        ...
        Dictionary<Guid, NativeReader> chunkCache = [];
        IterateNodes(state, streamingData, htree, hnode, mtree, mnode, [], chunkCache);
    ...
    private void IterateNodes(RenderCreateState state, TerrainStreamingTree tree, HeightfieldTree htree, HeightfieldTreeNode hnode, TerrainMaskTree mtree, TerrainMaskTreeNode mnode, List<LayerData> layerDatas, Dictionary<Guid, NativeReader> chunkCache)
    {
        ...
        if (hnode.children.Count != 0 /*|| mnode.children.Count != 0*/)
        {
            ...
            List<LayerData> tmpLayers = [.. layerDatas];
...
public struct TerrainVertex
{
    ...
    public Vector2 TexCoords;   // Texture coordinates (u, v)
    public Vector3 Color;       // Vertex color
}
...
public class TerrainChunkRenderable
{
    ...
    public List<Vector4> TexOffsets { get; private set; } = []; // Texture offsets
    public List<TerrainVertex> TerrainVertices = []; // Vertex list
    public List<uint> TerrainIndices = []; // Index list (triangular faces)

    /// <summary>
    /// HeightfieldTree node Id for this chunk (quadtree coordinates).
    /// </summary>
    public QuadTreeNodeId NodeId { get; private set; }

    /// <summary>
    /// HeightfieldTree nodeSamplesPerSide (dim), used to derive height sample index from vertex index.
    /// </summary>
    ...
    private List<BindableTextureWithData> textures = [];
    private static Random r = new Random();

    /// <summary>
    /// Whether vertex data has been edited externally; needs re-upload to GPU before next render.
    /// </summary>
    ...
    public TerrainChunkRenderable(RenderCreateState state, HeightfieldTree htree, HeightfieldTreeNode hnode, TerrainMaskTree mtree/*, TerrainMaskTreeNode mnode*/, List<TerrainRenderable.LayerData> layerDatas)
    {
            ...
            for (int nx = 0; nx < 4; nx++)
            {
                for (int y = 0; y < (dim / 4) + 1; y++)
                {
                    for (int x = 0; x < (dim / 4) + 1; x++)
                    {
                        ...
                        TerrainVertices.Add(new TerrainVertex()
                        {
                ...
                for (int y = 0; y < (dim / 4); y++)
                {
                    ...
                    {
                        TerrainIndices.Add((uint)(((y + 0) * ((dim / 4) + 1)) + (x + 0) + voffset));
                        TerrainIndices.Add((uint)(((y + 1) * ((dim / 4) + 1)) + (x + 1) + voffset));
                        TerrainIndices.Add((uint)(((y + 0) * ((dim / 4) + 1)) + (x + 1) + voffset));

                        TerrainIndices.Add((uint)(((y + 0) * ((dim / 4) + 1)) + (x + 0) + voffset));
                        TerrainIndices.Add((uint)(((y + 1) * ((dim / 4) + 1)) + (x + 0) + voffset));
                        TerrainIndices.Add((uint)(((y + 1) * ((dim / 4) + 1)) + (x + 1) + voffset));
                    }
                }

                voffset = TerrainVertices.Count;
            }
        }
        //int z = 0;
        //for (int y = 0; y < dim; y++)
        ...
        calculateNormals(TerrainIndices, ref TerrainVertices);
        vertexBuffer = Buffer.Create<TerrainVertex>(state.Device, BindFlags.VertexBuffer, TerrainVertices.ToArray());
        indexBuffer = Buffer.Create<uint>(state.Device, BindFlags.IndexBuffer, TerrainIndices.ToArray());
        ...
        List<Dictionary<AtlasTileId, byte[]>> layers = [];
        int z = 0;
            ...
            for (int j = 0; j < layer.Node.subtiles.Count; j++)
            {
                ...
                if (subtile.terrainLayerIndex == i)
                {
                    if ((subtile.flags & 2) == 0)
                    {
                        // 2026/01/18
                        // Change marker
                        if (!layers[subtile.terrainLayerIndex].ContainsKey(subtile.atlasTileId))
    ...
    /// <summary>
    /// Mark vertices as modified; will re-upload to GPU before next render.
    /// </summary>
    ...
    public void ExportToOBJ(string filepath)
    {
        ...
        // Output example: v 0.0 1.0 0.0
        foreach (TerrainVertex v in TerrainVertices)
        {
            // Format: v x y z
            sw.WriteLine("v {0} {1} {2}", v.Position.X, v.Position.Y, v.Position.Z);
        }

        // Texture coordinates (new)
        //foreach (TerrainVertex v in TerrainVertices)
        //{
        //    sw.WriteLine("vt {0} {1}", v.TexCoords.X, v.TexCoords.Y);
        //}

        // Write normals (vn)
        // Output example: vn 0.0 0.0 1.0
        foreach (TerrainVertex v in TerrainVertices)
        {
            // Format: vn nx ny nz
            sw.WriteLine("vn {0} {1} {2}", v.Normal.X, v.Normal.Y, v.Normal.Z);
        }

        // Write faces (f)
        for (int i = 0; i < TerrainIndices.Count; i += 3)
        {
            uint index1 = TerrainIndices[i] + 1; // OBJ index starts at 1
            uint index2 = TerrainIndices[i + 1] + 1;
            uint index3 = TerrainIndices[i + 2] + 1;

            uint normalIndex1 = index1;
            uint normalIndex2 = index2;
            uint normalIndex3 = index3;

            // Format: f vertex_index//normal_index
            // Format: f vertex_index/texcoord_index/normal_index
            // Output example: f 1//1 2//2 3//3
            sw.WriteLine("f {0}//{1} {2}//{3} {4}//{5}",
                index1, normalIndex1, index2, normalIndex2, index3, normalIndex3);
        }
    ...
    public void Render(DeviceContext context, MeshRenderPath renderPath)
    {
        ...
        {
            // If vertices were edited externally, re-upload to GPU before render
            if (verticesDirty)
            {
                vertexBuffer?.Dispose();
                calculateNormals(TerrainIndices, ref TerrainVertices);
                vertexBuffer = Buffer.Create<TerrainVertex>(context.Device, BindFlags.VertexBuffer, TerrainVertices.ToArray());
                verticesDirty = false;
            }
