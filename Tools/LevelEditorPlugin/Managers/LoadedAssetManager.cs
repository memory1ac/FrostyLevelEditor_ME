...
public class LoadedAssetManager
{
    ...
    private Dictionary<EbxImportReference, LoadedAssetInfo> m_loadedAssets = [];
    private Dictionary<Guid, LoadedAssetInfo> m_loadedEbx = [];

    private static Dictionary<Type, Type> m_assetTypes = [];
    ...
    public void UpdateAsset(Asset asset)
    {
        ...
        App.Logger.Log($"LoadedAssetManager UpdateAsset事件被触发：{entry.Name}");
    }
    ...
    public void UndoUpdate(Asset asset)
    {
        ...
        App.Logger.Log($"LoadedAssetManager UndoUpdate事件被触发：{entry.Name}");
    }
    ...
    private Asset CreateAsset(FrostySdk.Ebx.Asset assetData, Guid fileGuid)
    {
        ...
        if (assetData == null)
        {
            App.Logger.LogWarning("无法创建资源，AssetData为Null");
            return null;
        }
