...
{
    public List<PointerRef> DataList { get; private set; } = [];
...
public class PropertyGridExtension : DependencyObject
{
    ...
    private static void OnInjectMockData(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        FrostyPropertyGrid propertyGrid = d as FrostyPropertyGrid;
        List<FrostyPropertyGridItemData> childItems = [];
        ...
        else
        {
            ...
            foreach (PropertyInfo pi in pis)
            {
                ...
                {
                    Binding = new PropertyValueBinding(pi, e.NewValue)
                };
...
public class PropertiesViewModel : Controls.IDockableItem, INotifyPropertyChanged
{
    ...
    private static bool IsTransformRelatedPath(Frosty.Core.Controls.FrostyPropertyGridItemData item)
    {
        if (item == null) return false;
        string path = item.Path ?? "";
        string name = item.Name ?? "";
        if (path.IndexOf("Transform", StringComparison.OrdinalIgnoreCase) >= 0) return true;
        if (path.IndexOf("InstanceTransforms", StringComparison.OrdinalIgnoreCase) >= 0) return true;
        if (string.Equals(name, "Transform", StringComparison.OrdinalIgnoreCase)) return true;
        var p = item.Parent as Frosty.Core.Controls.FrostyPropertyGridItemData;
        while (p != null)
        {
            if (string.Equals(p.Name, "Transform", StringComparison.OrdinalIgnoreCase)) return true;
            p = p.Parent as Frosty.Core.Controls.FrostyPropertyGridItemData;
        }
        return false;
    }

    /// <summary>
    /// 从 binding 和层级结构中查找受影响的 StaticModelGroupElementEntity。
    /// 支持 level data 多重层级下，通过 binding 的 instance 或 list[index] 反查 entity。
    /// </summary>
    private static StaticModelGroupElementEntity TryFindAffectedStaticModelGroupElement(
        Frosty.Core.Controls.IValueBinding binding,
        IEditorProvider editorProvider)
    {
        if (binding == null || editorProvider?.RootLayer == null) return null;

        object targetObject = null;
        var bt = binding.GetType();

        if (bt.Name == "PropertyValueBinding")
        {
            var instField = bt.GetField("instance", BindingFlags.NonPublic | BindingFlags.Instance);
            targetObject = instField?.GetValue(binding);
        }
        else if (bt.Name == "ArrayItemValueBinding")
        {
            var listField = bt.GetField("list", BindingFlags.Public | BindingFlags.Instance);
            var idxField = bt.GetField("index", BindingFlags.Public | BindingFlags.Instance);
            var list = listField?.GetValue(binding) as IList;
            var idx = idxField != null ? (int)(idxField.GetValue(binding) ?? -1) : -1;
        ...
        editorProvider.RootLayer.CollectEntities(entities);

        foreach (var e in entities.OfType<StaticModelGroupElementEntity>())
        {
            if (ReferenceEquals(e.Data, targetObject)) return e;
            if (ReferenceEquals(e.Data?.Transform, targetObject)) return e;
        }
        return null;
    }
    ...
    private void PropertyGridModified(object args)
    {
        ...
        // 属性面板修改 StaticModelGroup 的 Transform 时，同步 Havok 内存并注册待写回。
        // 支持 level data 多重层级：通过 binding 反查受影响的 StaticModelGroupElementEntity。
        StaticModelGroupElementEntity affectedElement = null;
        if (m_selectedEntity is StaticModelGroupElementEntity smgElement && IsTransformRelatedPath(e.Item))
            affectedElement = smgElement;
        else if (IsTransformRelatedPath(e.Item) && m_owner is IEditorProvider provider)
            affectedElement = TryFindAffectedStaticModelGroupElement(e.Item.Binding, provider);

        if (affectedElement != null)
        {
            (affectedElement.Parent as StaticModelGroupEntity)?.UpdateData(affectedElement);
            if (e.Item.Parent?.DisplayName != PropertyGridExtension.PropertiesCategory)
            {
                SubWorldReferenceObject sw = affectedElement.FindAncestor<SubWorldReferenceObject>();
                if (sw != null)
                    LoadedAssetManager.Instance.UpdateAsset(sw.Blueprint);
            }
        }
