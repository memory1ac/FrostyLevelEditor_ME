    ...
    class ExportModMenuItemCommand : ICommand
    {
        ...
        public void Execute(object parameter)
        {
            ...
            if (win.DialogResult == true)
            {
                ...
                // 2026/01/20
                // 改动标记
                FrostySaveFileDialog sfd = new FrostySaveFileDialog("Save Mod", "*.fbmod (Frosty Mod)|*.fbmod", "Mod", win.modTitleTextBox.Text);
                if (sfd.ShowDialog())
                {
                    string filename = sfd.FileName;
                    FrostyTaskWindow.Show("Saving Mod", "", (task) =>
                    {
                        mainWin.ExportMod(mainWin.Project.GetModSettings(), filename, false);
                    });
                }
            }
        }
