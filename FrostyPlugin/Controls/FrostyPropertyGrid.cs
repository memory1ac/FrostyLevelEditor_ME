...
namespace Frosty.Core.Controls
{
    ...
    public class FrostyPropertyGridItem : TreeViewItem
    {
        ...
        // 2026/01/20
        // 改动标记
        private ContextMenu CreateContextMenu(FrostyPropertyGridItemData item)
        {
            ...
            // Paste - 粘贴引用
            {
                ...
                menuItem.Click += PasteMenuItem_Click;
                BindingOperations.SetBinding(menuItem, IsEnabledProperty, new Binding("HasData") { Source = FrostyClipboard.Current });
                _contextMenu.Items.Add(menuItem);
            }
            ...
            if (item.IsPointerRef)
            {
                ...
                // ★ 复制Guid
                {
                    ...
                    menuItem.Click += CopyGuidMenuItem_Click;
                    _contextMenu.Items.Add(menuItem);
                }
                // ★ 打印Guid
                {
                    ...
                    menuItem.Click += PrintGuidMenuItem_Click;
                    _contextMenu.Items.Add(menuItem);
                }

                var ptrRef = (PointerRef)item.Value;
                if (ptrRef.Type == PointerRefType.External)
                {
                    ...
                    // 打开资源
                    {
                        ...
                        menuItem.Click += (s, e) =>
                        {
                            var assetEntry = App.AssetManager.GetEbxEntry(ptrRef.External.FileGuid);
                            if (assetEntry == null)
                                return;
                            App.EditorWindow.OpenAsset(assetEntry);
                        };
                        _contextMenu.Items.Add(menuItem);
                    }
                    // 定位到资源浏览器
            ...
            // 打印选中信息
            {
                ...
                // 复制名称到剪切板
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (item == null || item.Value == null)
                            return;
                        FixClipboard.SetText(item.DisplayName);
                    };
                    _contextMenu.Items.Add(menuItem);
                }
                // 复制值到剪切板
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (item == null || item.Value == null)
                            return;
                        FixClipboard.SetText(item.Value.ToString());
                    };
                    _contextMenu.Items.Add(menuItem);
                }
                // 打印选中信息
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        ...
                        App.Logger.Log("--------------------------------------------------");
                        App.Logger.Log($"DisplayName: \t{item.DisplayName}");
                        App.Logger.Log($"IsModified: \t{item.IsModified}");
                        App.Logger.Log($"Name: \t\t{item.Name}");
                        App.Logger.Log($"Value: \t\t{item.Value}");
                        App.Logger.Log($"Path: \t\t{item.Path}");
                        App.Logger.Log($"Type: \t\t{item.Value.GetType().FullName}");
                        App.Logger.Log("--------------------------------------------------");
                    };
                    _contextMenu.Items.Add(menuItem);
                }
            ...
            if (item.DisplayName == "Flags" && item.Path != "Flags" && item.Value.GetType().Name == "UInt32")
            {
                _contextMenu.Items.Add(new Separator());
                // PropertyConnections.[5].Flags
                var isPropFlag = item.Path.StartsWith("PropertyConnections");
                ...
                // 计算Flags数据
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (uint.TryParse(item.Value.ToString(), out var flags))
                        {
                            var frostyFlagsWindow = new FrostyFlagsWindow(flags, isPropFlag)
                            {
                                Owner = Application.Current.MainWindow
                            };
                            if (frostyFlagsWindow.ShowDialog() == true)
                            {
                                item.Value = frostyFlagsWindow.Flags;
                            }
                        }
                    };
                    _contextMenu.Items.Add(menuItem);
                ...
                // 打印Flags信息
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (uint.TryParse(item.Value.ToString(), out var flags))
                        {
                            ...
                            {
                                var realm = flags & 7;
                                var propType = (flags & 48) >> 4;
                                var sourceCantBeStatic = (flags & 8) != 0 ? "✔️" : "❌";

                                var realmFlags_0 = realm == 0 ? "✔️" : "❌";
                                var realmFlags_1 = realm == 1 ? "✔️" : "❌";
                                var realmFlags_2 = realm == 2 ? "✔️" : "❌";
                                var realmFlags_3 = realm == 3 ? "✔️" : "❌";
                                var realmFlags_4 = realm == 4 ? "✔️" : "❌";
                                var realmFlags_5 = realm == 5 ? "✔️" : "❌";

                                var propertyFlags_0 = propType == 0 ? "✔️" : "❌";
                                var propertyFlags_1 = propType == 1 ? "✔️" : "❌";
                                var propertyFlags_2 = propType == 2 ? "✔️" : "❌";
                                var propertyFlags_3 = propType == 3 ? "✔️" : "❌";

                                App.Logger.Log("--------------------------------------------------");
                                App.Logger.Log($"0 - {realmFlags_0} Invalid");
                                App.Logger.Log($"1 - {realmFlags_1} ClientAndServer");
                                App.Logger.Log($"2 - {realmFlags_2} Client");
                                App.Logger.Log($"3 - {realmFlags_3} Server");
                                App.Logger.Log($"4 - {realmFlags_4} NetworkedClient");
                                App.Logger.Log($"5 - {realmFlags_5} NetworkedClientAndServer");
                                App.Logger.Log("----------");
                                App.Logger.Log($"0 - {propertyFlags_0} Default");
                                App.Logger.Log($"1 - {propertyFlags_1} Interface");
                                App.Logger.Log($"2 - {propertyFlags_2} Exposed");
                                App.Logger.Log($"3 - {propertyFlags_3} Invalid");
                                App.Logger.Log("----------");
                                App.Logger.Log($"0 - {sourceCantBeStatic} SourceCantBeStatic");
                                App.Logger.Log("--------------------------------------------------");
                            }
                            else
                            {
                                var unknown = (flags & 0x1000000u) != 0 ? "✔️" : "❌";
                                var clientEvent = (flags & 0x2000000u) != 0 ? "✔" : "❌";
                                var serverEvent = (flags & 0x4000000u) != 0 ? "✔" : "❌";
                                var clientProperty = (flags & 0x8000000u) != 0 ? "✔" : "❌";
                                var serverProperty = (flags & 0x10000000u) != 0 ? "✔" : "❌";
                                var clientLink = (flags & 0x20000000u) != 0 ? "✔" : "❌";
                                var serverLink = (flags & 0x40000000u) != 0 ? "✔" : "❌";
                                var unused = (flags & 0x80000000u) != 0 ? "✔" : "❌";

                                uint guidMask = flags & 0xFFFFFFu;

                                App.Logger.Log("--------------------------------------------------");
                                App.Logger.Log($"Guid Mask : {guidMask}");
                                App.Logger.Log("----------");
                                App.Logger.Log($"0 - {unknown} Unknown");
                                App.Logger.Log($"1 - {clientEvent} ClientEvent");
                                App.Logger.Log($"2 - {serverEvent} ServerEvent");
                                App.Logger.Log($"3 - {clientProperty} ClientProperty");
                                App.Logger.Log($"4 - {serverProperty} ServerProperty");
                                App.Logger.Log($"5 - {clientLink} ClientLink");
                                App.Logger.Log($"6 - {serverLink} ServerLink");
                                App.Logger.Log($"7 - {unused} Unused");
                                App.Logger.Log("--------------------------------------------------");
                            }
                        }
                    };
            ...
            if (item.DisplayName != "Flags" && (item.Value.GetType().Name == "Int32" || item.Value.GetType().Name == "UInt32"))
            {
                _contextMenu.Items.Add(new Separator());
                ...
                // 计算Hash数据
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (long.TryParse(item.Value.ToString(), out var hashId))
                        {
                            var fnvHashWindow = new FNVHashWindow((int)hashId)
                            {
                                Owner = Application.Current.MainWindow
                            };
                            if (fnvHashWindow.ShowDialog() == true)
                            {
                                if (item.Value.GetType().Name == "UInt32")
                                    item.Value = (uint)fnvHashWindow.HashId;
                                else
                                    item.Value = fnvHashWindow.HashId;
                            }
                        }
                    };
                    _contextMenu.Items.Add(menuItem);
                ...
                // 打印Hash信息
                {
                    ...
                    menuItem.Click += (s, e) =>
                    {
                        if (long.TryParse(item.Value.ToString(), out var hashId))
                        {
                            var hashString = Utils.GetString((int)hashId);

                            App.Logger.Log("--------------------------------------------------");
                            App.Logger.Log($"FNV Hash : {hashString} = {hashId}");
                            App.Logger.Log("--------------------------------------------------");
                        }
                    };
                    _contextMenu.Items.Add(menuItem);
            ...
            if (item.IsArrayChild)
            {
                ...
                // 改动标记
                {
                    var menuItem = new MenuItem { Header = "Insert Before - 在此项<前>插入" };
                    menuItem.Click += ArrayInsertBeforeMenuItem_Click;
                    _contextMenu.Items.Add(menuItem);
                }

                {
                    var menuItem = new MenuItem { Header = "Insert After - 在此项<后>插入" };
                    menuItem.Click += ArrayInsertAfterMenuItem_Click;
                    _contextMenu.Items.Add(menuItem);
                }
            }
            ...
            // 定位Ebx到资源浏览器
            {
                ...
                menuItem.Click += (s, e) =>
                {
                    var assetEntry = App.EditorWindow.GetOpenedAssetEntry();
                    if (assetEntry == null)
                        return;
                    App.EditorWindow.DataExplorer.SelectAsset(assetEntry);
                };
        ...
        {
            var item = (FrostyPropertyGridItemData)DataContext;
            var pointerRef = (PointerRef)item.Value;
            ...
            FixClipboard.SetText(guidToCopy);
        }
        ...
        /// 改动标记
        /// </summary>
        private void PrintGuidMenuItem_Click(object sender, RoutedEventArgs e)
        {
            App.Logger.Log("--------------------------------------------------");
            ...
            {
                App.Logger.Log("File Guid: \tNULL");
                App.Logger.Log("Class Guid: \tNULL");
            ...
            {
                App.Logger.Log("PointerRef Type External");
                App.Logger.Log($"File Guid: \t{pointerRef.External.FileGuid}");
                App.Logger.Log($"Class Guid: \t{pointerRef.External.ClassGuid}");
                App.Logger.Log($"Name: {App.AssetManager.GetEbxEntry(pointerRef.External.FileGuid).Name}");
            ...
            dynamic obj = pointerRef.Internal;
            App.Logger.Log("PointerRef Type Internal");
            App.Logger.Log($"File Guid: \t");
            App.Logger.Log($"Class Guid: \t{obj.GetInstanceGuid()}");
